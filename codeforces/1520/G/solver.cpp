...
void solve(Test& t)
{
	...
	map<pair<int, int>, long long> portals;
	...
	while (!q.empty()) {
		...
		if (pos.first == t.n-1 && pos.second == t.m-1) {
			cout << -node.first << endl;
			return;
		}
		
		if (already.count(pos) > 0)
			continue;
		already[pos] = true;
		
		//~ debug("POS", pos.first, pos.second, node.first);
		
		// for each standard edge
		for (int o : {-1, 1}) {
			for (bool isRow : {true, false}) {
				int newVal = (isRow ? pos.first : pos.second) + o;
				
				if (
					isRow &&
					newVal >= 0 && newVal < t.n &&
					t.a[newVal][pos.second] != -1 &&
					already.count({newVal, pos.second}) == 0
				)
					q.push({node.first - t.w, {newVal, pos.second}});
				
				if (
					!isRow &&
					newVal >= 0 && newVal < t.m &&
					t.a[pos.first][newVal] != -1 &&
					already.count({pos.first, newVal}) == 0
				)
					q.push({node.first - t.w, {pos.first, newVal}});
			}
		}
		
		// If we are in a portal then check every other portal
		if (t.a[pos.first][pos.second] > 0)
			for (auto& nextNode : portals)
				if (already.count({nextNode.first.first, nextNode.first.second}) == 0)
					q.push({node.first - t.a[pos.first][pos.second] - nextNode.second, {nextNode.first.first, nextNode.first.second}});
	}
