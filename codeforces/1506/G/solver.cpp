...
void solve(Test& t)
{
	...
	for (auto& c : t.s)
		++count[c - 0x61];
	vector<char> pre(t.s.size(), '\0');
	pre[t.s.size()-1] = t.s[t.s.size()-1];
	for (int k = (int)t.s.size()-2; k >= 0; --k) {
		pre[k] = pre[k+1];
		if (count[t.s[k] - 0x61] > 1)
			pre[k] = max(pre[k], t.s[k]);
		if (count[t.s[k] - 0x61] == 1)
			pre[k] = t.s[k];
	}
	...
	while (k < t.s.size()) {
		int c {t.s[k] - 0x61};
		if (count[c] > 1 && k < t.s.size()-1 && t.s[k] > pre[k+1]) {
			count[c] = -1;
			res += t.s[k];
		}
		if (count[c] == 1)
