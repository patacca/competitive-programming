...
long long expM(long long b, long long e, long long m) {
	long long res = 1;
	long long a = b;
	while (e) {
		if (e&1)
...
void solve(Test& t)
{
	// d d d d d --> ok (gcd = g, hence g | sum(di*(di+1)/2)
	// p...p d...d --> ok (gcd = g (odd), hence g | sum(di*(di+1)/2)
	// p p p p p --> the number of pi = gcd(pi)*di has to be even
	...
	for (int k=0; k < t.n; ++k) {
		if (t.a[k] % 2 == 0) {
			++evens;
			++b[__builtin_ctz(t.a[k])];
		} else {
			++odds;
		}
	}
	
	// all the subsequences containing odds numbers
	if (odds > 0) {
		long long partial = (expM(2, odds, m) - 1 + m) % m;
		sol = (partial * expM(2, evens, m)) % m;
	}
	
	// all the subsequences containing only even numbers
	for (auto it = b.begin(); it != b.end(); ++it) {
		evens -= it->second;
		long long partial = (expM(2, it->second-1, m) - 1 + m) % m;
		sol = (sol + partial * expM(2, evens, m)) % m;
	}
