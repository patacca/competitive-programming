...
void solve(Test& t)
{
	auto isPal = [&] (int ignore) {
		int i = 0;
		int j = t.n-1;
		while (i <= j) {
			if (t.a[i] == ignore)
				++i;
			else if (t.a[j] == ignore)
				--j;
			else if (t.a[i] == t.a[j]) {
				++i;
				--j;
			} else
				return make_pair(false, make_pair(t.a[i], t.a[j]));
		}
		
		return make_pair(true, make_pair(0, 0));
	};
	
	pair<bool, pair<int, int>> r = isPal(-1);
	if (r.first) {
		cout << "YES\n";
		return;
	}
	
	if (isPal(r.second.first).first)
		cout << "YES\n";
	else if (isPal(r.second.second).first)
		cout << "YES\n";
	else
		cout << "NO\n";
}
