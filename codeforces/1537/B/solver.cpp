...
long long check(long long x1, long long y1, long long x2, long long y2, long long px, long long py)
{
	long long dist = 0ll;
	dist += abs(x1 - px);
	dist += abs(y1 - py);
...
void solve(Test& t)
{
	pair<pair<int,int>, pair<int,int>> best {{1,1},{1,1}};
	long long maxDist = 0LL;
	for (int x1 : {0, t.n-1})
		for (int x2 : {0, t.n-1})
			for (int y1 : {0, t.m-1})
				for (int y2 : {0, t.m-1}) {
					long long dist = check(x1+1, y1+1, x2+1, y2+1, t.i, t.j);
					//~ debug(dist, maxDist);
					if (dist > maxDist) {
						maxDist = dist;
						best = {{x1+1, y1+1}, {x2+1, y2+1}};
					}
				}
	
	cout << best.first.first << " " << best.first.second << " " << best.second.first << " " << best.second.second << endl;
}
